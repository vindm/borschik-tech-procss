var INHERIT = require('inherit'),
    ...
    PluginsLoader = require('./plugins_loader'),

    PLUGINS_PATH = './lib/plugins/',
...
exports.Tech = INHERIT(cssbase.Tech, {
    ...
    __constructor: function(opts) {
        ...
        this.plugins = new PluginsLoader(PLUGINS_PATH).plugins;
    },
    ...
    Task: exports.Task = INHERIT({
        ...
        process: function() {
            ...
            try {
                plugin = this.tech.plugins[command];
                if ( ! plugin) {
                    throw new Error(command + ' plugin is not found.');
                }

                handler = plugin && plugin[this.type];
                if ( ! (handler && typeof handler === 'function')) {
                    throw new Error(this.type + ' handler is not defined in ' + command + ' plugin.');
                }
                ...
                if ( ! result) {
                    throw new Error(command + '#' + this.type + ' handler result is bad. Task: ' + this);
                }
            } catch (err) {
